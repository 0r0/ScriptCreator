...
public class CancelRequest implements Script {
    Sheet sheet;

    public  CancelRequest(Sheet sheet)
    {
        this.sheet=sheet;
    }
    ...
    public CancelRequest get() {
        return new CancelRequest(sheet);
    }
    ...
    public  String generateScript() throws IOException {
        ...
        String scriptName = config.getProperty("cancel_issue");//get the contracted script name
        String scriptFullAddress = scriptAddress + scriptName;

        String sheetName = sheetConfig.getProperty("cancel_issue");//get the sheet name of cancel issue excel file
...
//        String fieldB = fieldsConfig.getProperty("cancel_issue.Field.B.Header");//it is vouchar number
        ...
        StringBuilder CancelRequestScript = new StringBuilder();// a string for storing cancel issue script
        ...
        for (Row row : sheet) {
            ...
            if (checker(row, validationFields)) {
                ...
                CancelRequestScript.append(createScriptPerIssueNumber(issueNumber, scriptFullAddress));
                CancelRequestScript.append("\n");

            }

        }

        return CancelRequestScript.toString();
    }
    @Override
